...
use obsidian_cli_inspector::query::{
    diagnose_broken_links, get_dead_ends, get_orphans,
};
...
fn setup_large_db(conn: &Connection, num_notes: usize, num_links: usize) {
    ...
    for i in 0..num_notes {
        conn.execute(
            ...
            rusqlite::params![format!("note_{}.md", i), format!("Note {}", i), i as i64, format!("hash_{}", i)],
        )
        .unwrap();
    }
    ...
    for i in 0..orphan_count {
        conn.execute(
            "INSERT INTO notes (path, title, mtime, hash) VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params![format!("orphan_{}.md", i), format!("Orphan {}", i), i as i64, format!("orphan_hash_{}", i)],
        )
        .unwrap();
    }
    ...
    for i in 0..dead_end_count {
        ...
        conn.execute(
            "INSERT INTO notes (path, title, mtime, hash) VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params![format!("dead_end_{}.md", i), format!("Dead End {}", i), i as i64, format!("dead_hash_{}", i)],
        )
        .unwrap();
