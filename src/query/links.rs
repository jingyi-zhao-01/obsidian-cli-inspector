...
pub fn diagnose_broken_links(conn: &Connection) -> Result<Vec<BrokenLinkResult>> {
    ...
    let mut stmt_candidates = conn.prepare(
        "SELECT path FROM notes WHERE path = ?1 OR title = ?1 LIMIT 2",
    )?;
...
pub fn get_orphans(conn: &Connection, exclude_templates: bool, exclude_daily: bool) -> Result<Vec<DiagnoseResult>> {
...
/// Optionally exclude templates and daily notes
pub fn get_dead_ends(conn: &Connection, exclude_templates: bool, exclude_daily: bool) -> Result<Vec<DiagnoseResult>> {
    let mut query = String::from(
        "SELECT 
            n.id,
            n.path,
...
mod tests {
    ...
    fn test_broken_link_result_ambiguous() {
        let result = BrokenLinkResult {
            ...
            status: "ambiguous".to_string(),
            candidates: vec!["folder1/duplicate.md".to_string(), "folder2/duplicate.md".to_string()],
        };
